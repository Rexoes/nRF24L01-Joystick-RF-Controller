...
Full_Data data = {500, 500, 0, 0, 1, 1, 0,111};

void joystick_control();
void directionControl();
void toggleSwitchControl();
void button_control();
void potantiometerControl();
void show_nrf_data();
...
void potantiometerControl(){
  ...
  pot2 = map(analogRead(Pot2_Pin), 0, 1023, 0, 255);
  if(!((pot2 < (pot2_prev + pot2_sensvity)) && (pot2 > (pot2_prev - pot2_sensvity)))){
    pot2_prev = pot2;
    data.joystick_pot2 = pot2;
    radio.write(&data, sizeof(Full_Data));
    //show_nrf_data();
    //Serial.println("Potantiometer Value Is: " + String(pot2));
  }
}
